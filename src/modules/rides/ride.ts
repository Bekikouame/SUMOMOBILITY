...
type RideWithDriverProfile = Prisma.RideGetPayload<{
  include: {
    ...
    driver: { // ⬅️ L'inclusion du chauffeur
      include: { 
        user: true; 
        driverProfile: true; // ⬅️ AJOUT CRUCIAL pour que le type soit reconnu
      };
    };
  };
}>;
...
this.eventEmitter.emit('ride.requested', {
  ...
  clientId: ride.clientId,
  clientName: `${ride.client.user.firstName} ${ride.client.user.lastName}`,
  clientPhone: ride.client.user.phone, // AJOUTÉ
  ...
  baseFare: ride.baseFare, // 🆕 AJOUTÉ
  distanceKm: ride.distanceKm, // 🆕 AJOUTÉ
  durationMinutes: durationMinutes, // 🆕 AJOUTÉ
  passengerCount: ride.passengerCount, // 🆕 AJOUTÉ
  rideType: ride.rideType, // 🆕 AJOUTÉ
  notes: ride.notes, // 🆕 AJOUTÉ
  status: ride.status, // 🆕 AJOUTÉ
  requestedAt: ride.requestedAt, // 🆕 AJOUTÉ
...
return tx.ride.update({
  ...
  include: {
    ...
    //   }
    // });
console.log('✅ Course acceptée, notification via WebSocket');
    // Émettre événement
